    ...
    <section class="kpi-summary-grid">
      <div class="kpi-card kpi-emerald">
        <div class="kpi-icon-glow icon-emerald">
          <svg width="22" height="22" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>
        </div>
        <div class="kpi-info">
          <div class="kpi-num-row">
            <span class="kpi-num color-emerald">{{ formatCurrency(totalIncome) }}</span>
            <span class="kpi-mini-tag tag-emerald">Revenue</span>
          </div>
      ...
      <div class="kpi-card kpi-rose">
        <div class="kpi-icon-glow icon-rose">
          <svg width="22" height="22" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 17h8m0 0v-8m0 8l-8-8-4 4-6-6"/></svg>
        </div>
        <div class="kpi-info">
          <div class="kpi-num-row">
            <span class="kpi-num color-rose">{{ formatCurrency(totalExpense) }}</span>
            <span class="kpi-mini-tag tag-rose">Expenses</span>
          </div>
      ...
      <div class="kpi-card kpi-blue">
        ...
        <div class="kpi-info">
          <div class="kpi-num-row">
            <span class="kpi-num color-blue" :class="{ 'text-danger': netBalance < 0 }">{{ formatCurrency(netBalance) }}</span>
            <span class="kpi-mini-tag tag-blue">Margin</span>
          </div>
      ...
      <div class="kpi-card kpi-purple">
        <div class="kpi-icon-glow icon-purple">
          <svg width="22" height="22" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
        </div>
        <div class="kpi-info">
          <div class="kpi-num-row">
            <span class="kpi-num color-purple">{{ items.length }}</span>
            <span class="kpi-mini-tag tag-purple">Logs</span>
          </div>
            ...
            <div class="modal-content">
              ...
              <form @submit.prevent="saveItem">
                <div class="modal-body">
                  ...
                  <div class="form-row mb-3">
                    <div class="col">
                      ...
                      <CustomSelect
                        v-model="form.type"
                        :options="[
                          { value: 'Income', label: 'Income (+ Client Billing / Revenue)' },
                          { value: 'Expense', label: 'Expense (- Site / Material / Payroll)' }
                        ]"
                  ...
                  <div class="form-group mb-3">
                    <label class="form-label">Related Construction Project</label>
                    <CustomSelect
                      v-model="form.project_id"
                      :options="projectsList"
                      placeholder="General Overhead / Unassigned"
                    />
                  </div>
...
import { accountingApi, projectsApi } from '../services/api'
...
.kpi-purple, .kpi-blue, .kpi-emerald, .kpi-rose { background: #ffffff; border-color: #e2e8f0; }
...
.icon-purple, .icon-blue, .icon-emerald, .icon-rose { background: linear-gradient(135deg, #2563eb, #1d4ed8); box-shadow: 0 4px 12px rgba(37, 99, 235, 0.2); }
...
.color-purple, .color-blue, .color-emerald, .color-rose { color: #0f172a; }
...
.tag-purple, .tag-blue, .tag-emerald, .tag-rose { background: #eff6ff; color: #1d4ed8; border: 1px solid #dbeafe; }
