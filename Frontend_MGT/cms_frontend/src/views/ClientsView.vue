  ...
  <div class="view-container">
    ...
    <div class="view-header mb-4 d-flex justify-content-between align-items-center">
      <div>
        ...
        <p class="page-subtitle">Manage clients</p>
      </div>
      ...
      <div v-else-if="items.length" class="table-responsive">
        <table class="table table-hover align-middle mb-0">
          <thead class="table-light">
            <tr>
              <th>Company Name</th><th>Email</th><th>Phone</th><th>Address</th>
              <th>Actions</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td>{{ item.companyName }}</td><td>{{ item.email }}</td><td>{{ item.phone }}</td><td>{{ item.address }}</td>
              <td>
...
const saveItem = async () => {
  try {
    editingId.value ? await clientsApi.update(editingId.value, form.value) : await clientsApi.create(form.value)
    resetForm()
    load()
  } catch (error) {
    console.error('Failed:', error)
